...
window.setButtonLoading = function (buttonId, isLoading = true) {
    ...
    if (isLoading) {
        ...
        if (btn._originalContent) {
            btn.replaceChildren(...btn._originalContent.map((n) => n.cloneNode(true)));
        } else {
...
// Store each button's original child nodes (not as an HTML string) so
// restoring it later never re-parses text as HTML.
document.addEventListener('DOMContentLoaded', function () {
